...
export class DriverAuthGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const result = await this.driverAuthService.authorize(token);

    if (!result?.isAuthorized) {
      throw new UnauthorizedException('Driver is not authorized');
    }

    // attach data to request for downstream handlers
    request.driver = result.driver;
    request.driverSession = result.session;
    request.driverIsActive = result.isActive;
